        ...
        subscribe(branchUseCase.getBranches(), {
            mView?.run {
                ...
                preferences.branch?.let { branch ->
                    showData(it.data.toBranches(LatLng(preferences.orderLocation.lat, preferences.orderLocation.lng)).sortedBy { it.distance }, branch.id)
                } ?: kotlin.run {
                    showData(it.data.toBranches(LatLng(preferences.orderLocation.lat, preferences.orderLocation.lng)).sortedBy { it.distance }, -1)
                }
            }
        }, {
            it.printStackTrace()
